...
function helperScriptSource(): string {
  return [
    ...
    "if(anchor.orderNumber){detailTargets.push({mode:'ord_info',orderNumber:anchor.orderNumber,ordFlag:anchor.ordFlag||'0',label:anchor.text});continue;}",
    ...
    "const waitForDetailPage=(win,beforeHtml,beforeUrl,orderNumber)=>new Promise((resolve,reject)=>{const started=Date.now();const tick=()=>{try{if(!win||win.closed){reject(new Error('Midtown detail window closed before capture finished.'));return;}if(!win.document||win.document.readyState!=='complete'){setTimeout(tick,250);return;}const afterUrl=win.location.href;const afterHtml=win.document.documentElement.outerHTML;const afterText=(win.document.body&&win.document.body.innerText||'').toLowerCase();const changed=afterUrl!==beforeUrl||afterHtml!==beforeHtml;const looksLikeDetail=afterText.includes('item status')||afterText.includes('line total')||afterText.includes('cover artist')||afterHtml.includes('/product/');if(changed&&looksLikeDetail){resolve();return;}}catch(error){}if(Date.now()-started>15000){reject(new Error(`Timed out opening Midtown order ${orderNumber} details.`));return;}setTimeout(tick,250);};tick();});",
    "const captureOrdInfoTarget=async(target)=>{const detailWin=window.open(location.href,'_blank','noopener=no,width=1280,height=900');if(!detailWin){throw new Error('Allow pop-ups so Comicos can capture Midtown order details.');}await waitForLoaded(detailWin);const anchors=[...detailWin.document.querySelectorAll('a[href]')];const rawPattern=new RegExp(`ord_info\\\\(${target.orderNumber}\\\\s*,\\\\s*['\\\"]?${target.ordFlag||'0'}['\\\"]?\\\\)`,'i');const anchor=anchors.find((node)=>{const raw=node.getAttribute('href')||'';const text=(node.textContent||'').trim();return rawPattern.test(raw)||(text===target.label&&/^[0-9]{5,}$/.test(text));});if(!anchor){detailWin.close();throw new Error(`Midtown order ${target.orderNumber} link was not found on the opened page.`);}const beforeUrl=detailWin.location.href;const beforeHtml=detailWin.document.documentElement.outerHTML;anchor.dispatchEvent(new detailWin.MouseEvent('click',{bubbles:true,cancelable:true,view:detailWin}));await waitForDetailPage(detailWin,beforeHtml,beforeUrl,target.orderNumber);const detailUrl=detailWin.location.href;const html=detailWin.document.documentElement.outerHTML;detailWin.close();return {detail_url:detailUrl,fallback_order_number:target.orderNumber,html};};",
