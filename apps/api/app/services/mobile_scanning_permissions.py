...
from app.models import Organization, ScanEvent
    ...
    *,
    ...
    session.add(
        ScanEvent(
            ...
            event_type="unauthorized_scan_access_attempt",
...
def resolve_mobile_scan_permissions(
    session: Session,
    *,
    organization_id: int,
    actor_user_id: int,
...
def validate_mobile_scan_view_access(
    session: Session,
    *,
    organization_id: int,
    actor_user_id: int,
) -> MarketplacePermissionResolution:
    resolution = resolve_mobile_scan_permissions(session, organization_id=organization_id, actor_user_id=actor_user_id)
    if not resolution.can_view:
        _record_unauthorized_attempt(
            session,
            organization_id=organization_id,
            actor_user_id=actor_user_id,
            action="mobile_scan:view",
            reason=resolution.reason,
        )
        raise HTTPException(status_code=403, detail="Mobile scan visibility is denied for this organization.")
    return resolution


def validate_mobile_scan_manage_access(
    session: Session,
    *,
    organization_id: int,
    actor_user_id: int,
    ...
    if not resolution.can_manage:
        _record_unauthorized_attempt(
            ...
            organization_id=organization_id,
            actor_user_id=actor_user_id,
            action="mobile_scan:manage",
            reason=resolution.reason,
        )
        raise HTTPException(status_code=403, detail="Mobile scan management is denied for this organization.")
    return resolution
