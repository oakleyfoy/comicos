...
from app.models import OfflineInventoryEvent, Organization
    ...
    *,
    ...
    session.add(
        OfflineInventoryEvent(
            ...
            event_type="unauthorized_offline_inventory_access_attempt",
...
def resolve_offline_inventory_permissions(
    session: Session,
    *,
    organization_id: int,
    actor_user_id: int,
...
def validate_offline_inventory_view_access(
    session: Session,
    *,
    organization_id: int,
    actor_user_id: int,
) -> MarketplacePermissionResolution:
    resolution = resolve_offline_inventory_permissions(session, organization_id=organization_id, actor_user_id=actor_user_id)
    if not resolution.can_view:
        _record_unauthorized_attempt(
            session,
            organization_id=organization_id,
            actor_user_id=actor_user_id,
            action="offline_inventory:view",
            reason=resolution.reason,
        )
        raise HTTPException(status_code=403, detail="Offline inventory visibility is denied for this organization.")
    return resolution


def validate_offline_inventory_manage_access(
    session: Session,
    *,
    organization_id: int,
    actor_user_id: int,
    ...
    if not resolution.can_manage:
        _record_unauthorized_attempt(
            ...
            organization_id=organization_id,
            actor_user_id=actor_user_id,
            action="offline_inventory:manage",
            reason=resolution.reason,
        )
        raise HTTPException(status_code=403, detail="Offline inventory management is denied for this organization.")
    return resolution
