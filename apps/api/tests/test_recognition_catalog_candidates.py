...
def test_search_absolute_batman_imported_series(client: TestClient, session: Session) -> None:
    """ComicVine volume imports land in catalog_series/catalog_issue and must be searchable."""
    ...
    series = CatalogSeries(
        ...
        start_year=2024,
        volume_number=1,
        external_source_ids={"COMICVINE": {"160294": {}}},
    )
    session.add(series)
    session.flush()
    issue_ids: dict[str, int] = {}
    for number in range(1, 21):
        issue = CatalogIssue(
            ...
            issue_number=str(number),
            normalized_issue_number=normalize_issue_number(str(number)),
            title=f"Absolute Batman #{number}" if number == 1 else None,
        )
        session.add(issue)
        session.flush()
        session.add(
            CatalogImage(
                issue_id=issue.id,
                image_type="cover",
                source_url=f"https://example.com/absolute-batman-{number}.jpg",
                source="comicvine",
            )
        )
        issue_ids[str(number)] = int(issue.id)
    ...
    token = register_and_login(client, "candidates-absolute-batman@example.com")
    response = client.get(
        "/api/v1/recognition/catalog-candidates",
        headers=auth_headers(token),
        params={"q": "absolute batman", "limit": 24},
    )
    assert response.status_code == 200, response.text
    cards = response.json()
    assert len(cards) == 20
    assert cards[0]["series"] == "Absolute Batman"
    assert cards[0]["catalog_issue_id"] == issue_ids["1"]
    assert cards[0]["publisher"] == "DC Comics"
