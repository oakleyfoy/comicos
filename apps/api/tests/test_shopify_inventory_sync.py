...
from app.services.shopify_inventory_sync import sync_availability
...
def test_shopify_inventory_sync_updates_stub_quantity(client: TestClient) -> None:
    reset_shopify_stub_state()
    register_and_login(client, "shopify-sync@example.com")
    with Session(get_engine()) as session:
        ensure_marketplace_definitions(session)
        owner_id = _owner_id(session, "shopify-sync@example.com")
        connect_account(
            session,
            owner_id=owner_id,
            payload=ShopifyConnectRequest(
                account_name="Sync Shop",
                shop_domain="sync.myshopify.com",
                admin_api_token="shopify_valid_sync",
            ),
        )
        listing = create_listing(
            ...
            payload=MarketplaceListingCreate(
                listing_title="Shopify Sync Listing",
