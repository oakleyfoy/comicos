...
function helperScriptSource(): string {
  return [
    ...
    "const captureOrdInfoTarget=async(target)=>{const detailWin=window.open(location.href,'_blank','noopener=no,width=1280,height=900');if(!detailWin){throw new Error('Allow pop-ups so Comicos can capture Midtown order details.');}await waitForLoaded(detailWin);const rawPattern=new RegExp(`ord_info\\\\(${target.orderNumber}\\\\s*,\\\\s*['\\\"]?${target.ordFlag||'0'}['\\\"]?\\\\)`,'i');const findAnchor=(win)=>[...win.document.querySelectorAll('a[href]')].find((node)=>{const raw=node.getAttribute('href')||'';const text=(node.textContent||'').trim();return rawPattern.test(raw)||(text===target.label&&/^[0-9]{5,}$/.test(text));});let popupWin=null;const originalOpen=typeof detailWin.open==='function'?detailWin.open.bind(detailWin):null;if(originalOpen){detailWin.open=(...args)=>{popupWin=originalOpen(...args);return popupWin;};}const anchor=findAnchor(detailWin);const beforeUrl=detailWin.location.href;const beforeHtml=detailWin.document.documentElement.outerHTML;const waitForProgress=()=>new Promise((resolve)=>setTimeout(resolve,1200));const extractResult=async()=>{if(popupWin){await waitForLoaded(popupWin);await waitForDetailPage(popupWin,'',popupWin.location.href,target.orderNumber);const detailUrl=popupWin.location.href;const html=popupWin.document.documentElement.outerHTML;popupWin.close();detailWin.close();return {detail_url:detailUrl,fallback_order_number:target.orderNumber,html};}if(detailWin.location.href!==beforeUrl||detailWin.document.documentElement.outerHTML!==beforeHtml||isDetailHtml(detailWin)){await waitForDetailPage(detailWin,beforeHtml,beforeUrl,target.orderNumber);const detailUrl=detailWin.location.href;const html=detailWin.document.documentElement.outerHTML;detailWin.close();return {detail_url:detailUrl,fallback_order_number:target.orderNumber,html};}return null;};const attempts=[];if(anchor&&typeof anchor.click==='function'){attempts.push(()=>anchor.click());}if(anchor){attempts.push(()=>anchor.dispatchEvent(new detailWin.MouseEvent('click',{bubbles:true,cancelable:true,view:detailWin})));}if(typeof detailWin.ord_info==='function'){attempts.push(()=>detailWin.ord_info(target.orderNumber,target.ordFlag||'0'));}if(typeof target.rawHref==='string'&&target.rawHref.toLowerCase().startsWith('javascript:')&&typeof detailWin.eval==='function'){attempts.push(()=>detailWin.eval(target.rawHref.replace(/^javascript:/i,'')));}if(!attempts.length){if(originalOpen){detailWin.open=originalOpen;}detailWin.close();throw new Error(`Midtown order ${target.orderNumber} link was not found on the opened page.`);}try{for(const attempt of attempts){attempt();await waitForProgress();const result=await extractResult();if(result){return result;}}throw new Error(`Midtown order ${target.orderNumber} stayed on the orders page after every detail-link attempt.`);}finally{if(originalOpen){detailWin.open=originalOpen;}if(detailWin&&!detailWin.closed){detailWin.close();}}};",
