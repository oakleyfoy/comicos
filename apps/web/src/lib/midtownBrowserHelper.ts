...
function helperScriptSource(): string {
  return [
    ...
    "const captureOrdInfoTarget=async(target)=>{const detailWin=window.open(location.href,'_blank','noopener=no,width=1280,height=900');if(!detailWin){throw new Error('Allow pop-ups so Comicos can capture Midtown order details.');}await waitForLoaded(detailWin);const rawPattern=new RegExp(`ord_info\\\\(${target.orderNumber}\\\\s*,\\\\s*['\\\"]?${target.ordFlag||'0'}['\\\"]?\\\\)`,'i');const findAnchor=(win)=>[...win.document.querySelectorAll('a[href]')].find((node)=>{const raw=node.getAttribute('href')||'';const text=(node.textContent||'').trim();return rawPattern.test(raw)||(text===target.label&&/^[0-9]{5,}$/.test(text));});let popupWin=null;const originalOpen=typeof detailWin.open==='function'?detailWin.open.bind(detailWin):null;if(originalOpen){detailWin.open=(...args)=>{popupWin=originalOpen(...args);return popupWin;};}const anchor=findAnchor(detailWin);const beforeUrl=detailWin.location.href;const beforeHtml=detailWin.document.documentElement.outerHTML;try{if(anchor){anchor.dispatchEvent(new detailWin.MouseEvent('click',{bubbles:true,cancelable:true,view:detailWin}));}else if(typeof detailWin.ord_info==='function'){detailWin.ord_info(target.orderNumber,target.ordFlag||'0');}else if(typeof target.rawHref==='string'&&target.rawHref.toLowerCase().startsWith('javascript:')&&typeof detailWin.eval==='function'){detailWin.eval(target.rawHref.replace(/^javascript:/i,''));}else{throw new Error(`Midtown order ${target.orderNumber} link was not found on the opened page.`);}}finally{if(originalOpen){detailWin.open=originalOpen;}}if(popupWin){await waitForLoaded(popupWin);await waitForDetailPage(popupWin,'',popupWin.location.href,target.orderNumber);const detailUrl=popupWin.location.href;const html=popupWin.document.documentElement.outerHTML;popupWin.close();detailWin.close();return {detail_url:detailUrl,fallback_order_number:target.orderNumber,html};}await waitForDetailPage(detailWin,beforeHtml,beforeUrl,target.orderNumber);const detailUrl=detailWin.location.href;const html=detailWin.document.documentElement.outerHTML;detailWin.close();return {detail_url:detailUrl,fallback_order_number:target.orderNumber,html};};",
