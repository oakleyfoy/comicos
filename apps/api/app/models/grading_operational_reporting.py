"""P37-09 deterministic grading operational reporting registry."""
...
class GradingOperationalReportRun(SQLModel, table=True):
    __tablename__ = "grading_operational_report_run"
    __table_args__ = (
        UniqueConstraint("owner_user_id", "replay_key", name="uq_grading_operational_report_run_owner_replay"),
        SAIndex("ix_grading_operational_report_run_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_grading_operational_report_run_owner_status", "owner_user_id", "status", "id"),
        SAIndex("ix_grading_operational_report_run_type", "owner_user_id", "report_type", "id"),
    )
...
class GradingOperationalReportFile(SQLModel, table=True):
    __tablename__ = "grading_operational_report_file"
    __table_args__ = (
        SAIndex("ix_grading_operational_report_file_run", "grading_operational_report_run_id", "id"),
    )

    id: int | None = Field(default=None, primary_key=True)
    grading_operational_report_run_id: int = Field(
        foreign_key="grading_operational_report_run.id",
        nullable=False,
        index=True,
    )
...
class GradingOperationalReportItem(SQLModel, table=True):
    __tablename__ = "grading_operational_report_item"
    __table_args__ = (
        SAIndex("ix_grading_operational_report_item_run_row", "grading_operational_report_run_id", "row_number"),
    )
