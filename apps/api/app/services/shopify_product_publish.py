...
from app.schemas.shopify import ShopifyListingActionResponse
...
def publish_canonical_listing(session: Session, *, owner_id: int, listing_id: int) -> ShopifyListingActionResponse:
    ...
    if listing.status != LISTING_STATUS_READY:
        ...
        raise HTTPException(status_code=409, detail="Listing must be READY_TO_PUBLISH before Shopify publish.")
    ...
    if job.job.status == "FAILED":
        ...
        raise HTTPException(status_code=422, detail="Shopify publish validation failed.")
    ...
    return ShopifyListingActionResponse(
        listing_id=listing_id,
...
def _external_listing_id_for_mapping(session: Session, *, owner_id: int, listing_id: int) -> tuple[MarketplaceListingMapping, str]:
    ...
    if mapping is None or not mapping.external_listing_id:
        ...
        raise HTTPException(status_code=404, detail="Shopify listing mapping not found.")
    return mapping, mapping.external_listing_id


def update_canonical_listing(session: Session, *, owner_id: int, listing_id: int) -> ShopifyListingActionResponse:
...
def archive_listing(session: Session, *, owner_id: int, listing_id: int) -> ShopifyListingActionResponse:
    account = get_owner_shopify_account(session, owner_id=owner_id)
    mapping, external_id = _external_listing_id_for_mapping(session, owner_id=owner_id, listing_id=listing_id)
    connector = ShopifyConnector(marketplace_id=account.marketplace_id, account_id=account.id)
    connector.archive_listing(session, external_listing_id=external_id)
    updated = update_mapping_status(
        session,
        owner_id=owner_id,
        listing_id=listing_id,
...
def restore_listing(session: Session, *, owner_id: int, listing_id: int) -> ShopifyListingActionResponse:
    account = get_owner_shopify_account(session, owner_id=owner_id)
    mapping, external_id = _external_listing_id_for_mapping(session, owner_id=owner_id, listing_id=listing_id)
    connector = ShopifyConnector(marketplace_id=account.marketplace_id, account_id=account.id)
    connector.restore_listing(session, external_listing_id=external_id)
    updated = update_mapping_status(
        session,
        owner_id=owner_id,
        listing_id=listing_id,
