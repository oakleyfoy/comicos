...
import {
  ApiError,
  apiClient,
  type ScanCornerEdgeEvidenceRead,
  type ScanCornerEdgeRunDetail,
...
export function ScanCornerEdgesPage() {
  ...
  const [run, setRun] = useState<ScanCornerEdgeRunDetail | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await apiClient.listScanCornerEdgeRuns({ scan_image_id: selected.scan_image_id, limit: 1, offset: 0 });
        if (ignore || !response.items[0]) {
          if (!ignore) setRun(null);
          return;
        }
        const detail = await apiClient.getScanCornerEdgeRun(response.items[0].id);
        if (!ignore) setRun(detail);
      } catch (loadErr) {
        if (!ignore) setError(loadErr instanceof ApiError ? loadErr.message : "Unable to load corner/edge runs.");
      }
    })();
    return () => {
      ignore = true;
  ...
  const cornerPreview = useMemo(
    () => run?.artifacts.find((a) => a.artifact_type === "CORNER_REGION_PREVIEW")?.preview_data_url ?? run?.corner_region_preview_data_url ?? null,
    [run],
  );
  const edgePreview = useMemo(
    () => run?.artifacts.find((a) => a.artifact_type === "EDGE_REGION_PREVIEW")?.preview_data_url ?? run?.edge_region_preview_data_url ?? null,
    [run],
  );
  const overlayPreview = useMemo(
    () => run?.artifacts.find((a) => a.artifact_type === "CORNER_EDGE_OVERLAY")?.preview_data_url ?? null,
    [run],
  );
  const continuityPreview = useMemo(
    () => run?.artifacts.find((a) => a.artifact_type === "BORDER_CONTINUITY_MAP")?.preview_data_url ?? null,
    [run],
  );
  ...
  async function submitRun(): Promise<void> {
    ...
    setError(null);
    try {
      const detail = await apiClient.runScanCornerEdgeDetection({
        scan_image_id: selectedDefectRun.scan_image_id,
        defect_run_id: selectedDefectRun.id,
      });
      setRun(detail);
    } catch (runErr) {
      setError(runErr instanceof ApiError ? runErr.message : "Corner/edge detection failed.");
    } finally {
      setRunning(false);
    }
  }
  ...
  return (
    <AppShell>
      <PageHeader
        eyebrow="P40-08"
        title="Corner / Edge Wear Detection"
        description="Deterministic border deterioration evidence analysis."
        actions={
          <div className="flex gap-2">
            <Link to="/scan-defects" className="rounded-2xl border border-white/10 px-4 py-2 text-sm font-semibold text-slate-200">
              Defect foundation
            </Link>
            <Link to="/ops#scan-corner-edges-ops" className="rounded-2xl border border-amber-400/35 px-4 py-2 text-sm font-semibold text-amber-100">
              Ops diagnostics
            </Link>
          </div>
        }
      ...
      <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="grid gap-4 xl:grid-cols-[1fr,1.2fr]">
          <div className="space-y-4">
            ...
            >
              {running ? "Running corner/edge detection…" : "Run corner/edge detection"}
            </button>
            {run ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <PreviewCard title="Corner regions" src={cornerPreview} />
            <PreviewCard title="Edge regions" src={edgePreview} />
            <PreviewCard title="Wear overlay" src={overlayPreview} />
            <PreviewCard title="Border continuity" src={continuityPreview} />
          </div>
        </div>
      </section>
      ...
      {!run ? (
        <div className="mt-6">
          <EmptyState
            title="No corner/edge run loaded"
            description="Select a completed defect foundation run and execute corner/edge wear detection to inspect border deterioration evidence and replay-safe lineage."
          />
        </div>
      ) : (
        <>
          <section className="mt-6 grid gap-4 xl:grid-cols-[1.2fr,0.8fr]">
            <Panel title="Evidence table">
              {run.evidence.length === 0 ? (
                <p className="text-sm text-slate-500">No corner/edge wear evidence recorded.</p>
              ) : (
            ...
            <Panel title="Measurement panel">
              {selectedEvidence ? (
                <pre className="max-h-72 overflow-auto rounded-xl border border-white/10 bg-slate-950/50 p-3 text-[10px] text-amber-100">
          ...
          <section className="mt-6 grid gap-4 xl:grid-cols-2">
            ...
            <Panel title="Lineage / checksums">
              ...
              <ChecksumRow label="Corner/edge manifest" value={run.corner_edge_checksum} />
              ...
              <ul className="space-y-2 text-xs text-slate-300">
                {run.history.map((event) => (
                  <li key={event.id} className="rounded-xl border border-white/10 px-3 py-2">
                    ...
                    <p className="mt-1 font-mono text-[10px] text-amber-100">{event.event_checksum.slice(0, 16)}…</p>
                  </li>
                ))}
              </ul>
            </Panel>
...
function EvidenceRow({ evidence }: { evidence: ScanCornerEdgeEvidenceRead }): JSX.Element {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-slate-950/35 px-3 py-2 text-xs">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="font-semibold text-white">
            {evidence.evidence_type} · {evidence.region_type}
          </p>
          <p className="mt-1 text-slate-400">
            rank {evidence.evidence_rank} · {evidence.severity_hint} · confidence {evidence.confidence_score.toFixed(3)}
          </p>
        </div>
      ...
      <div className="mt-2 grid gap-2 md:grid-cols-3 text-slate-400">
        ...
        <span>break {Number(measurements.border_break_ratio ?? 0).toFixed(3)}</span>
        <span>continuity {Number(measurements.edge_continuity_score ?? 0).toFixed(3)}</span>
      </div>
    </div>
  );
}
