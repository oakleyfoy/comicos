...
ACQUISITION_CERTIFICATION_RESULTS = ("NOT_READY", "READY_WITH_WARNINGS", "APPROVED_FOR_PRODUCTION")


class AcquisitionCertificationRun(SQLModel, table=True):
    __tablename__ = "acquisition_certification_run"
    __table_args__ = (
        SAIndex("ix_acquisition_certification_run_started", "started_at", "id"),
        SAIndex("ix_acquisition_certification_run_result", "certification_result", "id"),
    )
    ...
    want_list_score: float = Field(default=0.0, nullable=False)
    collection_gap_score: float = Field(default=0.0, nullable=False)
    opportunity_score: float = Field(default=0.0, nullable=False)
    marketplace_score: float = Field(default=0.0, nullable=False)
