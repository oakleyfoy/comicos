...
EXIT_CERTIFICATION_RESULTS = ("NOT_READY", "READY_WITH_WARNINGS", "APPROVED_FOR_PRODUCTION")


class ExitCertificationRun(SQLModel, table=True):
    __tablename__ = "exit_certification_run"
    __table_args__ = (
        SAIndex("ix_exit_certification_run_started", "started_at", "id"),
        SAIndex("ix_exit_certification_run_result", "certification_result", "id"),
    )
    ...
    exit_candidate_score: float = Field(default=0.0, nullable=False)
    hold_sell_score: float = Field(default=0.0, nullable=False)
    grade_before_sell_score: float = Field(default=0.0, nullable=False)
    portfolio_rebalancing_score: float = Field(default=0.0, nullable=False)
