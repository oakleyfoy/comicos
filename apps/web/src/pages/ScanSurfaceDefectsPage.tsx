...
import {
  ...
  type ScanSurfaceDefectEvidenceRead,
  type ScanSurfaceDefectRunDetail,
} from "../api/client";
...
export function ScanSurfaceDefectsPage() {
  ...
  const [run, setRun] = useState<ScanSurfaceDefectRunDetail | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await apiClient.listScanSurfaceDefectRuns({ scan_image_id: selected.scan_image_id, limit: 1, offset: 0 });
        if (ignore || !response.items[0]) {
          if (!ignore) setRun(null);
          return;
        }
        const detail = await apiClient.getScanSurfaceDefectRun(response.items[0].id);
        if (!ignore) setRun(detail);
      } catch (loadErr) {
        if (!ignore) setError(loadErr instanceof ApiError ? loadErr.message : "Unable to load surface runs.");
      }
    })();
    return () => {
      ignore = true;
  ...
  const surfacePreview = useMemo(
    () => run?.artifacts.find((a) => a.artifact_type === "SURFACE_REGION_PREVIEW")?.preview_data_url ?? run?.surface_region_preview_data_url ?? null,
    [run],
  );
  const texturePreview = useMemo(
    () => run?.artifacts.find((a) => a.artifact_type === "SURFACE_TEXTURE_MAP")?.preview_data_url ?? null,
    [run],
  );
  const overlayPreview = useMemo(
    () => run?.artifacts.find((a) => a.artifact_type === "SURFACE_DEFECT_OVERLAY")?.preview_data_url ?? null,
    [run],
  );
  ...
  async function submitRun(): Promise<void> {
    ...
    setError(null);
    try {
      const detail = await apiClient.runScanSurfaceDefectDetection({
        scan_image_id: selectedDefectRun.scan_image_id,
        defect_run_id: selectedDefectRun.id,
      });
      setRun(detail);
    } catch (runErr) {
      setError(runErr instanceof ApiError ? runErr.message : "Surface defect detection failed.");
    } finally {
      setRunning(false);
    }
  }
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description="Deterministic surface-level evidence analysis."
        actions={
          <div className="flex gap-2">
            <Link to="/scan-defects" className="rounded-2xl border border-white/10 px-4 py-2 text-sm font-semibold text-slate-200">
              Defect foundation
            </Link>
            <Link to="/ops#scan-surface-defects-ops" className="rounded-2xl border border-fuchsia-400/35 px-4 py-2 text-sm font-semibold text-fuchsia-100">
              Ops diagnostics
            </Link>
          </div>
        }
      ...
      <section className="mt-6 rounded-3xl border border-white/10 bg-slate-900/65 p-5">
        <div className="grid gap-4 xl:grid-cols-[1fr,1.2fr]">
          <div className="space-y-4">
            ...
            >
              {running ? "Running surface detection…" : "Run surface defect detection"}
            </button>
            {run ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <PreviewCard title="Surface regions" src={surfacePreview} />
            <PreviewCard title="Texture map" src={texturePreview} />
            <PreviewCard title="Surface overlay" src={overlayPreview} />
          </div>
        </div>
      </section>
      ...
      {!run ? (
        <div className="mt-6">
          <EmptyState
            title="No surface run loaded"
            description="Select a completed defect foundation run and execute surface defect detection to inspect deterministic surface-level evidence and replay-safe lineage."
          />
        </div>
      ) : (
        <>
          <section className="mt-6 grid gap-4 xl:grid-cols-[1.2fr,0.8fr]">
            <Panel title="Evidence table">
              {run.evidence.length === 0 ? (
                <p className="text-sm text-slate-500">No surface evidence recorded.</p>
              ) : (
            ...
            <Panel title="Measurement panel">
              {selectedEvidence ? (
                <pre className="max-h-72 overflow-auto rounded-xl border border-white/10 bg-slate-950/50 p-3 text-[10px] text-fuchsia-100">
          ...
          <section className="mt-6 grid gap-4 xl:grid-cols-2">
            ...
            <Panel title="Lineage / checksums">
              ...
              <ChecksumRow label="Surface defect manifest" value={run.surface_defect_checksum} />
              ...
              <ul className="space-y-2 text-xs text-slate-300">
                {run.history.map((event) => (
                  <li key={event.id} className="rounded-xl border border-white/10 px-3 py-2">
                    ...
                    <p className="mt-1 font-mono text-[10px] text-fuchsia-100">{event.event_checksum.slice(0, 16)}…</p>
                  </li>
                ))}
              </ul>
            </Panel>
...
function EvidenceRow({ evidence }: { evidence: ScanSurfaceDefectEvidenceRead }): JSX.Element {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-slate-950/35 px-3 py-2 text-xs">
      <div className="flex items-start justify-between gap-2">
        ...
        <p className="font-mono text-[10px] text-fuchsia-100">
      ...
      <div className="mt-2 grid gap-2 md:grid-cols-3 text-slate-400">
        ...
        <span>brightness {Number(measurements.local_brightness_delta ?? 0).toFixed(3)}</span>
        <span>texture {Number(measurements.texture_variance ?? 0).toFixed(3)}</span>
      </div>
    </div>
  );
}
