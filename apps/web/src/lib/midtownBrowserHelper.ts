...
function helperScriptSource(): string {
  return [
    ...
    "const waitForDetailPage=(win,beforeHtml,beforeUrl,orderNumber)=>new Promise((resolve,reject)=>{const started=Date.now();const tick=()=>{try{if(!win||win.closed){reject(new Error('Midtown detail window closed before capture finished.'));return;}if(!win.document||win.document.readyState!=='complete'){setTimeout(tick,250);return;}const afterUrl=win.location.href;const afterHtml=win.document.documentElement.outerHTML;const changed=afterUrl!==beforeUrl||afterHtml!==beforeHtml;if(changed&&isDetailHtml(win)){resolve();return;}}catch(error){}if(Date.now()-started>15000){reject(new Error(`Timed out opening Midtown order ${orderNumber} details.`));return;}setTimeout(tick,250);};tick();});",
    "const captureOrdInfoTarget=async(target)=>{const originalOpen=window.open.bind(window);let openedWin=null;window.open=(...args)=>{openedWin=originalOpen(...args);return openedWin;};try{if(typeof ord_info==='function'){ord_info(target.orderNumber,target.ordFlag||'0');}else if(typeof target.rawHref==='string'&&target.rawHref.toLowerCase().startsWith('javascript:')){eval(target.rawHref.replace(/^javascript:/i,''));}else{throw new Error(`Midtown order ${target.orderNumber} detail action was unavailable on this page.`);}}finally{window.open=originalOpen;}if(!openedWin){throw new Error(`Midtown order ${target.orderNumber} did not open a detail window.`);}await waitForLoaded(openedWin);if(!openedWin.document||openedWin.document.readyState!=='complete'){throw new Error(`Timed out opening Midtown order ${target.orderNumber} details.`);}if(!isDetailHtml(openedWin)){const beforeUrl=openedWin.location.href;const beforeHtml=openedWin.document.documentElement.outerHTML;await waitForDetailPage(openedWin,beforeHtml,beforeUrl,target.orderNumber);}const detailUrl=openedWin.location.href;const html=openedWin.document.documentElement.outerHTML;openedWin.close();return {detail_url:detailUrl,fallback_order_number:target.orderNumber,html};};",
