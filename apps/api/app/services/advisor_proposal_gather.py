...
def _gather_sell_advisor(session: Session, *, owner_user_id: int) -> list[_Proposal]:
...
def _gather_grade_advisor(session: Session, *, owner_user_id: int) -> list[_Proposal]:
    out: list[_Proposal] = []
    rows = list(
        session.exec(
            select(P89SellCandidate)
    ...
    for row in rows:
        ...
        out.append(
            _Proposal(
                ...
                entity_id=int(row.id or 0),
                confidence=row.confidence,
                reason=row.reason_summary or "GRADE_FIRST",
...
def _gather_watch_advisor(session: Session, *, owner_user_id: int) -> list[_Proposal]:
    out: list[_Proposal] = []
    out.extend(_gather_release_alerts(session, owner_user_id=owner_user_id))
    future = list(
        session.exec(
    ...
    out.extend(_gather_collection_gaps(session, owner_user_id=owner_user_id))
    out.extend(_gather_portfolio_actions(session, owner_user_id=owner_user_id))
...
def gather_advisor_proposals(session: Session, *, owner_user_id: int) -> list[_Proposal]:
    ...
    gatherers = (
        _gather_buy_opportunities_advisor,
        _gather_marketplace_alerts_advisor,
        _gather_sell_advisor,
        _gather_grade_advisor,
        _gather_watch_advisor,
        _gather_p90_collector_alerts,
    )
    for gather in gatherers:
        try:
            proposals.extend(gather(session, owner_user_id=owner_user_id))
        except Exception as exc:  # noqa: BLE001
            logger.warning("advisor gather %s failed owner=%s: %s", gather.__name__, owner_user_id, exc)
            p90_rollback_session(session)
