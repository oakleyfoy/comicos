...
function helperScriptSource(): string {
  return [
    ...
    "const onOrderHistoryPage=(pageText.includes('orders in process')||pageText.includes('orders processed / shipped & completed')||pageText.includes('order date'))&&!location.pathname.includes('/view/');",
    "if(!onOrderHistoryPage){fail('Open the Midtown order history page in your account, then run the Comicos Midtown Sync bookmark.',syncData);return;}",
    ...
    ".filter((href)=>href&&(/\\/account-orders\\//.test(href)||/\\/account\\/orders\\//.test(href)||/\\/account-settings/.test(href)&&/#?order/i.test(href))&&!href.endsWith('/account/orders'));",
