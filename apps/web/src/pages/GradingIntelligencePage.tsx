...
export function GradingIntelligencePage(): JSX.Element {
  ...
  return (
    <AppShell>
      ...
      {loading ? <p className="text-sm text-slate-400">Loading grading intelligence…</p> : null}
              ...
              {!dashboard.recommendation_summary.length ? (
                ...
                <ul className="space-y-2 text-sm text-slate-300">
                  {dashboard.recommendation_summary.map((row) => (
                    <li key={row.id}>
                      <span className="font-medium text-white">{row.recommendation_type}</span> — {row.title}
                    </li>
                  ))}
                </ul>
              )}
