...
class ScanCornerEdgeRun(SQLModel, table=True):
    __tablename__ = "scan_corner_edge_run"
    __table_args__ = (
        UniqueConstraint("owner_user_id", "corner_edge_checksum", name="uq_scan_corner_edge_run_owner_checksum"),
        SAIndex("ix_scan_corner_edge_run_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_scan_corner_edge_run_scan_image", "scan_image_id", "created_at", "id"),
        SAIndex("ix_scan_corner_edge_run_defect", "defect_run_id", "created_at", "id"),
    )
    ...
    corner_edge_checksum: str = Field(max_length=64, nullable=False, index=True)
...
class ScanCornerEdgeEvidence(SQLModel, table=True):
    __tablename__ = "scan_corner_edge_evidence"
    __table_args__ = (
        SAIndex("ix_scan_corner_edge_evidence_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_scan_corner_edge_evidence_run_rank", "corner_edge_run_id", "evidence_rank", "id"),
        SAIndex("ix_scan_corner_edge_evidence_run_conf", "corner_edge_run_id", "confidence_score", "id"),
    )

    id: int | None = Field(default=None, primary_key=True)
    owner_user_id: int = Field(foreign_key="user.id", nullable=False, index=True)
    corner_edge_run_id: int = Field(foreign_key="scan_corner_edge_run.id", nullable=False, index=True)
    ...
    corner_overlap_ratio: float = Field(default=0.0, nullable=False)
    measurement_json: dict = Field(default_factory=dict, sa_column=Column(JSON, nullable=False))
    metadata_json: dict = Field(default_factory=dict, sa_column=Column(JSON, nullable=False))
    created_at: datetime = Field(default_factory=utc_now, sa_column=Column(DateTime(timezone=True), nullable=False))


class ScanCornerEdgeArtifact(SQLModel, table=True):
    __tablename__ = "scan_corner_edge_artifact"
    __table_args__ = (
        UniqueConstraint(
            "corner_edge_run_id",
            "artifact_type",
            "artifact_checksum",
            name="uq_scan_corner_edge_art_run_type_checksum",
        ),
        SAIndex("ix_scan_corner_edge_art_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_scan_corner_edge_art_run_type", "corner_edge_run_id", "artifact_type", "id"),
    )
...
class ScanCornerEdgeIssue(SQLModel, table=True):
    __tablename__ = "scan_corner_edge_issue"
    __table_args__ = (
        SAIndex("ix_scan_corner_edge_issue_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_scan_corner_edge_issue_run_type", "corner_edge_run_id", "issue_type", "id"),
    )
...
class ScanCornerEdgeHistory(SQLModel, table=True):
    __tablename__ = "scan_corner_edge_history"
    __table_args__ = (
        SAIndex("ix_scan_corner_edge_hist_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_scan_corner_edge_hist_run_type", "corner_edge_run_id", "event_type", "id"),
    )
