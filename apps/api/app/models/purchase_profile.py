...
class PurchasePreference(SQLModel, table=True):
    ...
    ratio_variant_strategy: str = Field(default="conservative", max_length=24, nullable=False)
    max_ratio_variant_price: float = Field(default=25.0, nullable=False)
    high_ratio_exception_required: bool = Field(default=True, nullable=False)
    high_ratio_threshold: int = Field(default=50, nullable=False)
    created_at: datetime = Field(default_factory=utc_now, sa_column=Column(DateTime(timezone=True), nullable=False))
    updated_at: datetime = Field(default_factory=utc_now, sa_column=Column(DateTime(timezone=True), nullable=False))
