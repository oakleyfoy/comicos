"""Quick vision stream for barcode-primary images skips GPT."""
...
def test_barcode_primary_stream_skips_gpt(tmp_path, monkeypatch) -> None:
    ...
    with Session(engine) as session:
        ...
        with mock.patch(
            ...
            with mock.patch(
                ...
                events = list(iter_vision_read_sse(session, image_id=7, force=True))

    gpt_stream.assert_not_called()
    assert any("barcode_primary" in e for e in events)
