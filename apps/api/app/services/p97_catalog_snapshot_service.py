...
def _build_publisher_index(session: Session) -> dict[str, CatalogPublisher]:
    index: dict[str, CatalogPublisher] = {}
    for row in session.exec(select(CatalogPublisher)).all():
        index[row.normalized_name] = row
    return index


def _build_series_index(session: Session) -> tuple[dict[str, CatalogSeries], dict[tuple[str, int | None], CatalogSeries]]:
...
def _build_issue_index(session: Session) -> tuple[dict[str, CatalogIssue], dict[tuple[int, str], CatalogIssue]]:
...
def _find_publisher(session: Session, by_norm: dict[str, CatalogPublisher], payload: dict[str, Any]) -> CatalogPublisher | None:
    ext = payload.get("external_source_ids") or {}
    for cv in comicvine_external_ids(ext):
        for row in session.exec(select(CatalogPublisher)).all():
            if cv in comicvine_external_ids(row.external_source_ids):
                return row
...
def import_catalog_snapshot(
    ...
    *,
    ...
    pub_by_norm = _build_publisher_index(session)
    series_by_volume, series_by_norm_pub = _build_series_index(session)
    issues_by_cv, issues_by_series_number = _build_issue_index(session)
    ...
    for record in publishers:
        ...
        if not key:
            stats.skipped += 1
            continue
        existing = _find_publisher(session, pub_by_norm, payload)
    ...
    for record in series_rows:
        ...
        if not publisher_id and pub_key:
            ...
            existing_pub = _find_publisher(session, pub_by_norm, pub_payload)
