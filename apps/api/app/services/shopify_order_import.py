...
from app.schemas.shopify import ShopifyImportOrdersResponse
...
def map_external_order(session: Session, *, owner_id: int, external_order: dict) -> MarketplaceOrderCreate:
    ...
    items = [
        MarketplaceOrderItemCreate(
            ...
            title=str(item.get("title") or "Shopify Product"),
    ...
    if not items:
        raise HTTPException(status_code=422, detail="Shopify order import requires at least one item.")
...
def import_orders(session: Session, *, owner_id: int) -> ShopifyImportOrdersResponse:
    account = get_owner_shopify_account(session, owner_id=owner_id)
    ...
    return ShopifyImportOrdersResponse(
