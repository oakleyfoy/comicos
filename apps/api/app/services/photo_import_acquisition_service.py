...
def create_catalog_copy_from_vision_read(
    ...
    *,
    ...
    if catalog_issue_id is None:
        # Not in the master catalog (e.g. a modern series we don't carry yet).
        # Capture it as a placeholder copy from GPT's read so intake isn't blocked.
        title = (read.series or read.issue_title or "").strip()
        if not title:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Cannot add to inventory: no catalog match and no series/title from the photo",
            )
