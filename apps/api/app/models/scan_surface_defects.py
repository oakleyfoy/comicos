...
class ScanSurfaceDefectRun(SQLModel, table=True):
    __tablename__ = "scan_surface_defect_run"
    __table_args__ = (
        UniqueConstraint("owner_user_id", "surface_defect_checksum", name="uq_scan_surface_defect_run_owner_checksum"),
        SAIndex("ix_scan_surface_defect_run_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_scan_surface_defect_run_scan_image", "scan_image_id", "created_at", "id"),
        SAIndex("ix_scan_surface_defect_run_defect", "defect_run_id", "created_at", "id"),
    )
    ...
    surface_defect_checksum: str = Field(max_length=64, nullable=False, index=True)
...
class ScanSurfaceDefectEvidence(SQLModel, table=True):
    __tablename__ = "scan_surface_defect_evidence"
    __table_args__ = (
        SAIndex("ix_scan_surface_defect_evidence_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_scan_surface_defect_evidence_run_rank", "surface_defect_run_id", "evidence_rank", "id"),
        SAIndex("ix_scan_surface_defect_evidence_run_conf", "surface_defect_run_id", "confidence_score", "id"),
    )

    id: int | None = Field(default=None, primary_key=True)
    owner_user_id: int = Field(foreign_key="user.id", nullable=False, index=True)
    surface_defect_run_id: int = Field(foreign_key="scan_surface_defect_run.id", nullable=False, index=True)
    ...
    surface_area_ratio: float = Field(default=0.0, nullable=False)
    measurement_json: dict = Field(default_factory=dict, sa_column=Column(JSON, nullable=False))
    metadata_json: dict = Field(default_factory=dict, sa_column=Column(JSON, nullable=False))
    created_at: datetime = Field(default_factory=utc_now, sa_column=Column(DateTime(timezone=True), nullable=False))


class ScanSurfaceDefectArtifact(SQLModel, table=True):
    __tablename__ = "scan_surface_defect_artifact"
    __table_args__ = (
        UniqueConstraint(
            "surface_defect_run_id",
            "artifact_type",
            "artifact_checksum",
            name="uq_scan_surface_defect_art_run_type_checksum",
        ),
        SAIndex("ix_scan_surface_defect_art_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_scan_surface_defect_art_run_type", "surface_defect_run_id", "artifact_type", "id"),
    )
...
class ScanSurfaceDefectIssue(SQLModel, table=True):
    __tablename__ = "scan_surface_defect_issue"
    __table_args__ = (
        SAIndex("ix_scan_surface_defect_issue_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_scan_surface_defect_issue_run_type", "surface_defect_run_id", "issue_type", "id"),
    )
...
class ScanSurfaceDefectHistory(SQLModel, table=True):
    __tablename__ = "scan_surface_defect_history"
    __table_args__ = (
        SAIndex("ix_scan_surface_defect_hist_owner_created", "owner_user_id", "created_at", "id"),
        SAIndex("ix_scan_surface_defect_hist_run_type", "surface_defect_run_id", "event_type", "id"),
    )
