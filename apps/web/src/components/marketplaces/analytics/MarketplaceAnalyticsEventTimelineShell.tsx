import type { MarketplaceAnalyticsEventResponse } from "../../../api/client";
import { MarketplaceAnalyticsStatusBadge } from "./MarketplaceAnalyticsStatusBadge";

export function MarketplaceAnalyticsEventTimelineShell({
  events,
}: {
    ...
    <section className="rounded-3xl border border-white/10 bg-slate-900/65 p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-base font-semibold text-white">Analytics event timeline</h2>
        </div>
        <p className="text-sm text-slate-400">{events.length} visible</p>
      </div>
      {events.length === 0 ? (
        <p className="mt-4 text-sm text-slate-400">No analytics lineage events have been recorded yet.</p>
      ) : (
          ...
          {events.map((event) => (
            <article key={event.id} className="rounded-2xl border border-white/10 bg-slate-950/45 p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <MarketplaceAnalyticsStatusBadge status="generated" />
              </div>
