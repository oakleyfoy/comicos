import type { MobileDeviceSecurityEventResponse } from "../../../api/client";

export function MobileSecurityEventTimeline({ events }: { events: MobileDeviceSecurityEventResponse[] }): JSX.Element {
  return (
    <section className="rounded-3xl border border-white/10 bg-slate-900/65 p-5">
      <div>
        <h2 className="text-lg font-semibold text-white">Security events</h2>
        <p className="mt-1 text-sm text-slate-400">Append-only device security lineage across trust changes, policy edits, and access decisions.</p>
      </div>
        ...
        {events.map((event) => (
          <div key={event.id} className="rounded-2xl border border-white/10 bg-slate-950/45 p-4">
            ...
            <p className="mt-2 text-xs text-slate-400">
              device {event.mobile_device_id ?? "n/a"} / actor {event.actor_user_id ?? "n/a"}
            </p>
