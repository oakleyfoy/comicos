...
function helperScriptSource(): string {
  return [
    ...
    "const captureOrdInfoTarget=async(target)=>{const detailWin=window.open(location.href,'_blank','noopener=no,width=1280,height=900');if(!detailWin){throw new Error('Allow pop-ups so Comicos can capture Midtown order details.');}await waitForLoaded(detailWin);if(typeof detailWin.ord_info!=='function'){detailWin.close();throw new Error('Midtown detail helper was not available on the opened page.');}detailWin.ord_info(target.orderNumber,target.ordFlag||'0');await waitForLoaded(detailWin);const detailUrl=detailWin.location.href;const html=detailWin.document.documentElement.outerHTML;detailWin.close();return {detail_url:detailUrl,fallback_order_number:target.orderNumber,html};};",
