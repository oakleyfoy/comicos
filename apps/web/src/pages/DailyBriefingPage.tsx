...
export function DailyBriefingPage(): JSX.Element {
  ...
  useEffect(() => {
    void (async () => {
      try {
        setBrief(await apiClient.getDailyBriefing());
      } catch (err) {
        setError(err instanceof ApiError ? err.message : "Failed to load daily briefing.");
      }
    })();
  }, []);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <header className="border-b border-slate-800 px-4 py-4">
        <div className="mx-auto max-w-3xl space-y-3">
          <h1 className="text-xl font-semibold">Daily collector briefing</h1>
      ...
      <main className="mx-auto max-w-3xl space-y-4 px-4 py-6 text-sm">
        ...
        {brief ? (
          <>
            <h2 className="font-semibold">Top actions today</h2>
