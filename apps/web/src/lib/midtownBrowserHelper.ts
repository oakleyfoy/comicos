...
function helperScriptSource(): string {
  return [
    ...
    "const orderAnchors=[...document.querySelectorAll('a[href]')].map((anchor)=>{const href=abs(anchor.getAttribute('href')||'');const text=(anchor.textContent||'').trim();return {href,text};})",
    ".filter(({href,text})=>href&&new URL(href).hostname.includes('midtowncomics.com')&&href!==location.href&&!/view all/i.test(text)&&((/^[0-9]{5,}$/.test(text))||/\\/account-orders\\//.test(href)||/\\/account\\/orders\\//.test(href)||(/\\/account-settings/.test(href)&&(/order/i.test(text)||/#?order/i.test(href)))));",
    "const links=orderAnchors.map(({href})=>href);",
    "const detailUrls=[...new Set(links)].slice(0,Number(syncData.limitOrders)||25);",
    "if(!detailUrls.length){fail('No Midtown order detail links were found. Make sure the My Orders table is fully visible before running the bookmark.',syncData);return;}",
